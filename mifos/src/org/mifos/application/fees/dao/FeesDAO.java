...
import org.mifos.application.fees.util.valueobjects.FeePayment;
...
public class FeesDAO extends DAO{
	...
	public void obtainMasterData(Context context,Short localeId)throws SystemException,ApplicationException {
		MasterDataRetriever masterDataRetriever=new MasterDataRetriever();
		context.addAttribute(masterDataRetriever.retrieveMasterData(FeesConstants.FEECATEGORY,localeId,FeesConstants.CATAGORY,"org.mifos.application.fees.util.valueobjects.CategoryType","categoryId"));
		context.addAttribute(masterDataRetriever.retrieveMasterData(FeesConstants.FEEPAYMENT, localeId,FeesConstants.PAYMENT ,"org.mifos.application.fees.util.valueobjects.FeePayment","feePaymentId"));
		context.addAttribute(masterDataRetriever.retrieveMasterData(FeesConstants.FEEFORMULA,localeId,FeesConstants.FORMULA ,"org.mifos.application.fees.util.valueobjects.FeeFormula","feeFormulaId"));
		context.addAttribute(masterDataRetriever.retrieveMasterData(FeesConstants.FEESTATUS,localeId,FeesConstants.STATUS ,"org.mifos.application.fees.util.valueobjects.FeeStatus","statusId"));
		...
		try
		{
			...
			Query query = session.getNamedQuery("masterdata.feepayment");
  		  	...
  		  	query.setShort("paymentId",FeesConstants.UPFRONT);
		  	List<FeePaymentMaster> viewFeePaymentForLoan = query.list();
		  	context.addBusinessResults("feePaymentForLoan",viewFeePaymentForLoan);
		}
	...
	public Fees getFees(Short feesId) throws SystemException,ApplicationException
	{
		Session session = null;
		Fees fees = null;
		try
	...
	private void initializeMeetings(Fees fees) {
		...
		if(fees.getFeeFrequency().getFeeFrequencyTypeId().equals(FeesConstants.PERIODIC)){
	...
	public Fees updateFees(Fees fees) throws SystemException,ApplicationException
	{
		Session session = null;
		...
		try
		{
			...
			retrivedFees.setStatus(fees.getStatus()); // if fee becomes inactive , validations
			retrivedFees.setRateOrAmount(fees.getRateOrAmount());
			if(retrivedFees.getRateFlatFalg().equals(Short.valueOf(FeesConstants.RATE))) 
				retrivedFees.setRate(fees.getRateOrAmount());
			else 
				retrivedFees.setFeeAmount(new Money(fees.getRateOrAmount().toString()));
			...
			tx.rollback();
			throw new ApplicationException(FeesConstants.VERSIONNOMATCHINGPROBLEM);
		}
		catch(HibernateProcessException e)
		{
			tx.rollback();
	...
	private List getGLCodes()throws SystemException,ApplicationException{
		...
		GLCodeEntity glCode;
		FinancialActionBO finActionFees = FinancialActionCache.getFinancialAction(FinancialActionConstants.FEEPOSTING);
		Set<COABO> applicableCreditCategory = finActionFees.getApplicableCreditCOA();
