...
/**
 * This class is for savings account related actions like creating, retrieving, updating savings account etc.
 */
public class SavingsAction extends AccountAppAction {
	...
	public SavingsAction() throws ServiceException{
		...
		savingsPrdService=(SavingsPrdBusinessService) ServiceFactory.getInstance().getBusinessService(BusinessServiceName.SavingsProduct);
	}
	...
	protected boolean skipActionFormToBusinessObjectConversion(String method)  {
		if(method.equals("edit")|| method.equals("create")|| method.equals("update")|| method.equals("previous")||method.equals("get")||
				method.equals("editPrevious") || method.equals("getRecentActivity") || method.equals("getTransactionHistory") ){
			logger.debug("In SavingsAction::skipActionFormToBusinessObjectConversion(), Skipping for Method: "+ method);
			return true;
		}
		else
			return false;
	...
	/**
	 * This method gets the applicable product offerings to be displayed in the UI for the selected customer. 
	 */
	public ActionForward getPrdOfferings(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)throws Exception {
		logger.debug("In SavingsAction::getPrdOfferings(), customerId: "+((SavingsActionForm)form).getCustomerId());
		doCleanUp(form,request);
		CustomerBO customer = getCustomer(new Integer(((SavingsActionForm)form).getCustomerId()));
		SavingsBO savings =(SavingsBO)request.getSession().getAttribute(Constants.BUSINESS_KEY);
		savings.setCustomer(customer);
		if(customer.getCustomerMeeting()!=null && customer.getCustomerMeeting().getMeeting()!=null)
			Hibernate.initialize(customer.getCustomerMeeting().getMeeting());
		List<PrdOfferingView> savingPrds = savingsService.getSavingProducts(customer.getOffice(),customer.getCustomerLevel(),SavingsConstants.SAVINGS_ALL);
		SessionUtils.setAttribute(SavingsConstants.SAVINGS_PRD_OFFERINGS,savingPrds,request.getSession());
		logger.info("In SavingsAction::getPrdOfferings(), Retrieved "+ savingPrds.size() + " Products for customerId: "+ customer.getCustomerId());
		return mapping.findForward(AccountConstants.GET_PRDOFFERINGS_SUCCESS);
	}
	
	public ActionForward load(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)throws Exception {
		logger.debug("In SavingsAction::load(), selectedPrdOfferingId: "+ ((SavingsActionForm)form).getSelectedPrdOfferingId());
		SavingsBO savings =(SavingsBO)SessionUtils.getAttribute(Constants.BUSINESS_KEY,request.getSession());
		loadAndSetSavingsOffering(new Short(((SavingsActionForm)form).getSelectedPrdOfferingId()),savings);
		UserContext uc = (UserContext)SessionUtils.getAttribute(Constants.USER_CONTEXT_KEY,request.getSession());
		SessionUtils.setAttribute(MasterConstants.INTEREST_CAL_TYPES,masterDataService.getMasterData(MasterConstants.INTEREST_CAL_TYPES,uc.getLocaleId(),"org.mifos.application.master.util.valueobjects.InterestCalcType","interestCalculationTypeID"),request.getSession());
		SessionUtils.setAttribute(MasterConstants.SAVINGS_TYPE,masterDataService.getMasterData(MasterConstants.SAVINGS_TYPE,uc.getLocaleId(),"org.mifos.application.master.util.valueobjects.SavingsType","savingsTypeId"),request.getSession());
		SessionUtils.setAttribute(MasterConstants.RECOMMENDED_AMOUNT_UNIT,masterDataService.getMasterData(MasterConstants.RECOMMENDED_AMOUNT_UNIT,uc.getLocaleId(),"org.mifos.application.master.util.valueobjects.RecommendedAmntUnit","recommendedAmntUnitId"),request.getSession());
		SessionUtils.setAttribute(SavingsConstants.CUSTOM_FIELDS,savingsService.retrieveCustomFieldsDefinition(),request.getSession());
		logger.info("In SavingsAction::load(), Data loaded successfully ");
		return mapping.findForward("load_success");
	}
	
	public ActionForward reLoad(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)throws Exception {
		logger.debug("In SavingsAction::reLoad(), selectedPrdOfferingId: "+ ((SavingsActionForm)form).getSelectedPrdOfferingId());
		SavingsBO savings =(SavingsBO)SessionUtils.getAttribute(Constants.BUSINESS_KEY,request.getSession());
		loadAndSetSavingsOffering(new Short(((SavingsActionForm)form).getSelectedPrdOfferingId()),savings);
	...
	private void loadAndSetSavingsOffering(Short savingsOfferingId , SavingsBO savings)throws Exception{
		SavingsOfferingBO savingsOffering=savingsPrdService.getSavingsProduct(savingsOfferingId);
	...
	public ActionForward preview(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)throws Exception {
		...
		SavingsBO savings =(SavingsBO)request.getSession().getAttribute(Constants.BUSINESS_KEY);
		SessionUtils.setAttribute(SavingsConstants.IS_PENDING_APPROVAL,Configuration.getInstance().getAccountConfig(savings.getCustomer().getOffice().getOfficeId()).isPendingApprovalStateDefinedForSavings(),request.getSession());
	...
	public ActionForward create(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)throws Exception {
		logger.debug("In SavingsAction::create(), accountStateId: "+ ((SavingsActionForm)form).getStateSelected());
		SavingsBO savings =(SavingsBO)SessionUtils.getAttribute(Constants.BUSINESS_KEY,request.getSession());
		savings.setAccountState(new AccountStateEntity(new Short(((SavingsActionForm)form).getStateSelected())));
		savings.save();
	...
	public ActionForward get(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)throws Exception {
		...
		logger.debug("In SavingsAction::get(), Retrieving for globalAccountNum: "+actionForm.getGlobalAccountNum());
		...
		SessionUtils.setAttribute(SavingsConstants.CUSTOM_FIELDS,savingsService.retrieveCustomFieldsDefinition(),request.getSession());
		SessionUtils.setAttribute(MasterConstants.RECOMMENDED_AMOUNT_UNIT,masterDataService.getMasterData(MasterConstants.RECOMMENDED_AMOUNT_UNIT,uc.getLocaleId(),"org.mifos.application.master.util.valueobjects.RecommendedAmntUnit","recommendedAmntUnitId"),request.getSession());
		SessionUtils.setAttribute(SavingsConstants.RECENTY_ACTIVITY_DETAIL_PAGE,savings.getRecentAccountActivity(3),request.getSession());
		logger.info("In SavingsAction::get(), Savings object retrieved successfully");
	...
	public ActionForward edit(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)throws Exception {
		...
		if(savings.isDepositScheduleBeRegenerated())
			SessionUtils.setAttribute(SavingsConstants.RECOMMENDED_AMOUNT,savings.getRecommendedAmount(),request.getSession());
		return mapping.findForward("edit_success");
	}
	
	public ActionForward editPreview(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)throws Exception {
		logger.debug("In SavingsAction::editPreview()");
		return mapping.findForward("editPreview_success");
	}
	
	public ActionForward editPrevious(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)throws Exception {
	...
	public ActionForward update(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)throws Exception {
		...
		SavingsBO savings =(SavingsBO)SessionUtils.getAttribute(Constants.BUSINESS_KEY,request.getSession());
		if(savings.isDepositScheduleBeRegenerated()){
			Money oldAmount=(Money)SessionUtils.getAttribute(SavingsConstants.RECOMMENDED_AMOUNT,request.getSession());
			if (oldAmount.getAmountDoubleValue()!=savings.getRecommendedAmount().getAmountDoubleValue())
	...
	public ActionForward getRecentActivity(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)throws Exception {
		logger.debug("In SavingsAction::getRecentActivity()");
		//Check for order-by clause in AccountBO.hbm.xml and AccountPayment.hbm.xml for accountPaymentSet and 
		//accountTrxnSet. It should be set for the primay key column desc in both. If stated is not there, the code 
		//below will behave abnormally.
		UserContext uc = (UserContext)SessionUtils.getAttribute(Constants.USER_CONTEXT_KEY,request.getSession());
		SavingsBO savings =(SavingsBO)SessionUtils.getAttribute(Constants.BUSINESS_KEY,request.getSession());
		savings = savingsService.findById(savings.getAccountId());
		savings.setUserContext(uc);
		SessionUtils.setAttribute(SavingsConstants.RECENTY_ACTIVITY_LIST,savings.getRecentAccountActivity(null),request.getSession());
		return mapping.findForward("getRecentActivity_success");
	}	
	
	public ActionForward getTransactionHistory(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)throws Exception {
		logger.debug("In SavingsAction::getRecentActivity()");
		Short localeId =((UserContext)SessionUtils.getAttribute(Constants.USER_CONTEXT_KEY,request.getSession())).getLocaleId();
		//SavingsBO savings =(SavingsBO)SessionUtils.getAttribute(Constants.BUSINESS_KEY,request.getSession());
		...
		//Check for order-by clause in AccountBO.hbm.xml, AccountPayment.hbm.xml and AccountTrxnEntity.hbm.xml for accountPaymentSet , 
		//accountTrxnSet and financialBoSet. They all should be set for their primay key column desc in both. If stated is not there, the code 
		//below will behave abnormally. 
		...
		for(AccountPaymentEntity accountPaymentEntity : accountPaymentSet){
			...
			for(AccountTrxnEntity accountTrxnEntity : accountTrxnEntitySet){
				...
				for(FinancialTransactionBO financialTransactionBO : financialTransactionBOSet){
					...
					savingsTransactionHistoryView.setType(financialTransactionBO.getFinancialAction().getName(localeId));
					savingsTransactionHistoryView.setGlcode(financialTransactionBO.getGlcode().getGlcode());
					if(financialTransactionBO.isDebitEntry()){
						savingsTransactionHistoryView.setDebit(String.valueOf(removeSign(financialTransactionBO.getPostedAmount())));
					}else if(financialTransactionBO.isCreditEntry()){
						savingsTransactionHistoryView.setCredit(String.valueOf(removeSign(financialTransactionBO.getPostedAmount())));
					}
					savingsTransactionHistoryView.setBalance(String.valueOf(removeSign(((SavingsTrxnDetailEntity)accountTrxnEntity).getBalance())));
		...
		SessionUtils.setAttribute(SavingsConstants.TRXN_HISTORY_LIST,savingsTransactionHistoryViewList,request.getSession());
	...
	public ActionForward getDepositDueDetails(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)throws Exception {
	...
	public ActionForward waiveAmountDue(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)throws Exception {
	...
	public ActionForward waiveAmountOverDue(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)throws Exception {
	...
	private void clearActionForm(ActionForm form){
		...
		for (AccountCustomFieldEntity customField: actionForm.getAccountCustomFieldSet()){
	...
	private void doCleanUp(ActionForm form, HttpServletRequest request){
		...
		request.getSession().removeAttribute(Constants.BUSINESS_KEY);
		UserContext uc = (UserContext)SessionUtils.getAttribute(Constants.USER_CONTEXT_KEY,request.getSession());
		SessionUtils.setAttribute(Constants.BUSINESS_KEY,new SavingsBO(uc),request.getSession());
	}
