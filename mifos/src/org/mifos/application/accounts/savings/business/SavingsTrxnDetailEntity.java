...
public class SavingsTrxnDetailEntity extends AccountTrxnEntity{
	...
	public void setTrxnDetails(Short action, Money amount, Money balance, CustomerBO customer, PersonnelBO createdBy)throws SystemException{
		this.setAccountActionEntity(getAccountDBService().getAccountAction(action));
		...
		else if (action.equals(AccountConstants.ACTION_SAVINGS_INTEREST_POSTING))
			this.setInterestAmount(amount);
		setTrxnCreatedDate(new Timestamp(System.currentTimeMillis()));
	}
	
	public void setTrxnDetails(Short action, Money amount, Money balance, CustomerBO customer, PersonnelBO createdBy, java.util.Date dueDate, java.util.Date transactionDate)throws SystemException{
		setTrxnDetails(action,amount,balance,customer,createdBy);
		setActionDate(transactionDate);
		setDueDate(dueDate);
	}
	
	private AccountPersistanceService getAccountDBService()throws ServiceException{
		return (AccountPersistanceService) ServiceFactory.getInstance().getPersistenceService(PersistenceServiceName.Account);
	}
	
	public void setPaymentDetails(Money depositAmount, 
			Date actionDate,CustomerBO customer,Short personnelId,java.util.Date transactionDate) throws ServiceException {
		...
		setAccountActionEntity((AccountActionEntity)masterPersistenceService.findById(AccountActionEntity.class,AccountConstants.ACTION_SAVINGS_DEPOSIT));
	...
	public void setWithdrawalDetails(Money amount, 
			Date actionDate,CustomerBO customer,Short personnelId,java.util.Date transactionDate) throws ServiceException {
		MasterPersistenceService masterPersistenceService=(MasterPersistenceService)ServiceFactory.getInstance().getPersistenceService(PersistenceServiceName.MasterDataService);
		...
		setDueDate(actionDate);
		setPersonnel(personnel);
		setAccountActionEntity((AccountActionEntity)masterPersistenceService.findById(AccountActionEntity.class,AccountConstants.ACTION_SAVINGS_WITHDRAWAL));
		setCustomer(customer);
		setTrxnCreatedDate(new Timestamp(System.currentTimeMillis()));
	...
	public AccountTrxnEntity generateReverseTrxn(String adjustmentComment)	throws ApplicationException, SystemException {
		...
		if (getAccountActionEntity().getId().equals(AccountConstants.ACTION_SAVINGS_DEPOSIT)){
			balAfterAdjust=getBalance().subtract(getDepositAmount());
			reverseAccntTrxn.setDepositAmount(getDepositAmount().negate());
		}else if (getAccountActionEntity().getId().equals(AccountConstants.ACTION_SAVINGS_WITHDRAWAL)){
			balAfterAdjust=getBalance().add(getWithdrawlAmount());
			reverseAccntTrxn.setWithdrawlAmount(getWithdrawlAmount().negate());
		}
		reverseAccntTrxn.setTrxnDetails(AccountConstants.ACTION_SAVINGS_ADJUSTMENT,getAmount().negate(),balAfterAdjust,getCustomer(),getPersonnel(),getDueDate(),getActionDate());
