...
public class PersonnelActionForm extends MifosSearchActionForm {
	...
	/**
	 * The supportedlocales object which identifies the preferred locale of the
	 * user.
	 */
	...
	/**
	 * Constructor: All the composition objects are intialized within the
	 * contructor
	 */
	...
	/**
	 * This method is used in addition to validation framework to do input data
	...
	public ActionErrors customValidate(ActionMapping mapping,
		...
		if (null != methodCalled) {
			if ((PersonnelConstants.METHOD_CANCEL).equals(methodCalled)
					...
					|| (PersonnelConstants.METHOD_SEARCH_NEXT)
							.equals(methodCalled)
					|| (PersonnelConstants.METHOD_SEARCH_PREV)
							.equals(methodCalled)
					|| (PersonnelConstants.METHOD_MANAGE).equals(methodCalled)
					|| (PersonnelConstants.METHOD_PREVIOUS)
							.equals(methodCalled)
					|| (PersonnelConstants.METHOD_LOAD_SEARCH)
							.equals(methodCalled)
					|| (PersonnelConstants.METHOD_LOAD_UNLOCK_USER)
							.equals(methodCalled)
					|| (PersonnelConstants.METHOD_UNLOCK_USER_ACCOUNT)
							.equals(methodCalled)
					|| (PersonnelConstants.METHOD_UPDATE).equals(methodCalled)
					|| (PersonnelConstants.METHOD_GET_DETAILS)
							.equals(methodCalled)
					|| (PersonnelConstants.METHOD_EDIT_PERSONAL_INFO)
							.equals(methodCalled)
					|| (PersonnelConstants.METHOD_PREV_PERSONAL_INFO)
							.equals(methodCalled)
					|| (PersonnelConstants.METHOD_CHOOSE_OFFICE)
							.equals(methodCalled)
					|| (PersonnelConstants.METHOD_LOAD_CHANGE_PASSWORD)
							.equals(methodCalled)
					|| (PersonnelConstants.METHOD_UPDATE_SETTINGS)
							.equals(methodCalled)
					|| ((PersonnelConstants.METHOD_SEARCH).equals(methodCalled)
							&& input != null && input
							.equals(PersonnelConstants.USER_CHANGE_LOG))) {
				request.setAttribute(Constants.SKIPVALIDATION, Boolean
						.valueOf(true));
			} else if ((PersonnelConstants.METHOD_PREVIEW).equals(methodCalled)
					&& input.equals(PersonnelConstants.CREATE_USER)) {
				...
				return handleSearchValidations(request);
					...
					&& input.equals(PersonnelConstants.MANAGE_USER)) {
	...
	/**
	 * This method is helper method to do data validations before searching for
	 * groups. If user has not entered any search String, it will display error
	 * message
	 * 
	 * @return ActionErrors
	 */
	private ActionErrors handleSearchValidations(HttpServletRequest request) {
		...
		if (ValidateMethods.isNullOrBlank(searchString)) {
			...
			errors.add(PersonnelConstants.NO_SEARCH_STRING, new ActionMessage(
					PersonnelConstants.NO_SEARCH_STRING));
		}
	 ...
	 * @param personnelStatus
	 */
	private ActionErrors handleManagePreviewValidations(
			HttpServletRequest request) {
		ActionErrors errors = null;
		if (errors == null)
			errors = new ActionErrors();
		checkForMandatoryFields(EntityMasterConstants.Personnel, errors,
				request);
	...
	private ActionErrors checkForPassword(ActionErrors errors) {
		...
		if (password.length() == passwordRepeat.length()
				&& password.length() != 0 && password.trim().equals("")) {
			if (errors == null)
				errors = new ActionErrors();
