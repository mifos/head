 ...
 * CollSheetLnDetailsEntity.java    version: 1.0
...
public class CollSheetLnDetailsEntity extends PersistentObject {
	...
	public Money getAmntToCloseLoan() {
		return getTotalPrincipalDue().add(getInterestDue()).add(getInterestOverDue()).add(getFeesDue()).add(getFeesOverDue()).add(getPenaltyDue()) .add(getPenaltyOverDue());
	}
	...
	public Money getTotalAmntOverDue() {
		...
		return new Money(Configuration.getInstance().getSystemConfig().getCurrency(),0).add(getPrincipalOverDue()).add(getInterestOverDue()).add( getFeesOverDue()).add(getPenaltyOverDue());
	}
	...
	/**
	 * This method sets account details from accountActionDate.This method also calculates over due amount
	 * till the current installment and adds them to the respedctive fields.Overdue amount for an installment is the 
	 * amount due till the (installment-1)
	 */
	public void addAccountDetails(AccountActionDateEntity accountActionDate)throws SystemException,ApplicationException {
		...
		LoanBO loan = collSheetPersistService.getLoanAccount( accountActionDate.getAccount().getAccountId());
		this.accountId = accountActionDate.getAccount().getAccountId();
		this.currentInstallmentNo = accountActionDate.getInstallmentId();
		MifosLogManager.getLogger(LoggerConstants.COLLECTIONSHEETLOGGER).debug("installment no i n accounts actions date is 4$$$$$$$$$$$$$$$$$$" + accountActionDate.getInstallmentId());
		this.principalDue = accountActionDate.getPrincipalDue();
		this.interestDue = accountActionDate.getInterestDue();
		this.penaltyDue = accountActionDate.getPenaltyDue().add( accountActionDate.getMiscPenalty());
		this.originalLoanAmnt = loan.getLoanAmount();
		this.feesDue = accountActionDate.getTotalFeeDue().add( accountActionDate.getMiscFeeDue());
		OverDueAmounts overDueAmounts =loan.getOverDueAmntsUptoInstallment(accountActionDate.getInstallmentId()); 
...
/**
	...
	public boolean equals(Object obj){
		...
		}else if(null != this.collectionSheetCustomer && null != collectionSheetLoanDetailsObj.getCollectionSheetCustomer()){
			return this.accountId.equals(collectionSheetLoanDetailsObj.getAccountId()) && this.collectionSheetCustomer.equals(collectionSheetLoanDetailsObj.getCollectionSheetCustomer()); 
		}else{
	...
	/**
	 * This sets accountId,amntToBeDisbursed and sets installment id to null because installments haven't begun yet.
	 * @param loan
	 */
	public void addDisbursalDetails(LoanBO loan) {
		...
		// TODO check in case of interest deducted at disbursement this should show the loan amount to be disbursed.
		this.amntToBeDisbursed = loan.getLoanAmount();
		if(loan.isInterestDeductedAtDisbursement()){
			this.interestDue = loan.getAccountActionDate(CollectionSheetConstants.FIRST_INSTALLMENT).getInterestDue();
			this.feesDue = loan.getAccountActionDate(CollectionSheetConstants.FIRST_INSTALLMENT).getTotalFeeDue();
		}
