...
public class CollSheetLnDetailsEntity extends PersistentObject {
	...
	public Money getAmntToCloseLoan() {
		return getTotalPrincipalDue().add(getInterestDue()).add(
				getInterestOverDue()).add(getFeesDue()).add(getFeesOverDue())
				.add(getPenaltyDue()).add(getPenaltyOverDue());
	}
	...
	public void addAccountDetails(AccountActionDateEntity accountActionDate)
		...
		this.penaltyDue = loanSchedule.getPenaltyDue().add(
				loanSchedule.getMiscPenalty());
		this.originalLoanAmnt = loan.getLoanAmount();
		this.feesDue = loanSchedule.getTotalFeeDue().add(
				loanSchedule.getMiscFeeDue());
		OverDueAmounts overDueAmounts = loan
				.getOverDueAmntsUptoInstallment(loanSchedule.getInstallmentId());
		...
		this.totalPrincipalDue = this.originalLoanAmnt.subtract(overDueAmounts
