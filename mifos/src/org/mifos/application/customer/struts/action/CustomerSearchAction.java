...
public class CustomerSearchAction extends MifosSearchAction {
	...
	public Map<String, String> appendToMap() {
		...
		customerMap.put(CustomerConstants.GETHOMEPAGE, CustomerConstants.GETHOMEPAGE);
		customerMap.put(CustomerConstants.LOADALLBRANCHES, CustomerConstants.LOADALLBRANCHES);
		customerMap.put(CustomerConstants.GET_OFFICE_HOMEPAGE, CustomerConstants.GET_OFFICE_HOMEPAGE);
		return customerMap;
	}
	...
	public ActionForward getHomePage(ActionMapping mapping, ActionForm form,
			...
			throws Exception {
		MifosLogManager.getLogger(LoggerConstants.CUSTOMERSEARCHLOGGER).debug("getHomePage method is called");
		boolean isCenterHeirarchyExists = Configuration.getInstance().getCustomerConfig(
				new OfficePersistenceService().getHeadOffice()
						.getOfficeId()).isCenterHierarchyExists();
		request.getSession().setAttribute(BulkEntryConstants.ISCENTERHEIRARCHYEXISTS,
	...
	public ActionForward load(ActionMapping mapping,
		...
		Short loadFordward=(Short)context.getBusinessResults(CustomerSearchConstants.LOADFORWARD);
		MifosLogManager.getLogger(LoggerConstants.CUSTOMERSEARCHLOGGER).info(
			"Inside Load method of CustomerSearchAction loadFordward"+loadFordward);
		if(loadFordward.equals(CustomerSearchConstants.LOADFORWARDLOANOFFICER)) {
			actionForward=mapping.findForward(CustomerSearchConstants.LOADFORWARDLOANOFFICER_SUCCESS);
		}
		else if(loadFordward.equals(CustomerSearchConstants.LOADFORWARDNONLOANOFFICER)) {
			actionForward=mapping.findForward(CustomerSearchConstants.LOADFORWARDNONLOANOFFICER_SUCCESS);
		} 
		else {
			actionForward=mapping.findForward(CustomerSearchConstants.LOADFORWARDOFFICE_SUCCESS);
		}
	...
	public ActionForward getOfficeHomePage(ActionMapping mapping,
		...
		if(loggedUserLevel.shortValue() == PersonnelConstants.LOAN_OFFICER.shortValue()){
	...
	/**
	 * 
	 * It returns an action forward based on the searchParameter coming from the UI which can be obatined from the 
	 * search node map.
	 * @param mapping
	 * @param form
	 * @param request
