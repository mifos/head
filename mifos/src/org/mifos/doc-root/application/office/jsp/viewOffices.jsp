				...
				<tr>
					...
					<table width="95%" border="0" cellpadding="3" cellspacing="0">
						...
						<tr>
							...
							<c:forEach
								var="office" items="${headOfficeList}">
								<!-- for head office -->
								<c:if test="${office.level.id == OfficeLevel.HEADOFFICE.value }">
									<span class="fontnormalbold">
										<html-el:link href="javascript:getOffice(${office.officeId})">
							...
							<c:choose>
								...
								<c:forEach	var="office" items="${regionalOfficeList}"> 	
										<!-- for regional  office -->
										<c:if test="${office.level.id == OfficeLevel.REGIONALOFFICE.value && empty regional}">
										...
										<table width="90%" border="0" cellspacing="0" cellpadding="0">
												<tr class="fontnormal">
													...
													<td width="99%">
													  ...
													  <c:if test="${office.status.id == OfficeStatus.INACTIVE.value}">
							...
							<c:choose>
								<c:when test="${empty divisionalOfficeList}">
									...
									<table width="95%" border="0" cellspacing="0" cellpadding="0">
											<tr>
												...
												<td width="39%" align="right">
												<html-el:link
													href="javascript:addNewOffice(${OfficeLevel.SUBREGIONALOFFICE})">
													<mifos:mifoslabel name="office.labelAddNew"
														bundle="OfficeResources"/>											
															<mifos:mifoslabel name="Office.labelDivisionalOffice"
														bundle="OfficeResources"/>
								...
								<c:forEach	var="office" items="${divisionalOfficeList}"> 	
										<!-- for regional  office -->
										<c:if test="${office.level.id == OfficeLevel.SUBREGIONALOFFICE.value && empty regional}">
											<c:set var="regional" value="level"/>
											<br>
											<table width="95%" border="0" cellspacing="0" cellpadding="0">
												<tr>
										...
										<c:if test="${office.level.id == OfficeLevel.SUBREGIONALOFFICE.value &&  empty divisional}">
							...
							<c:choose>
								...
								<c:forEach	var="office" items="${areaOfficeList}"> 	
										<!-- for regional  office -->
										<c:if test="${office.level.id == OfficeLevel.AREAOFFICE.value && empty area }">
							...
							<c:set var="branch"  />
							<c:forEach var="office" items="${branchOfficeList}" varStatus="counter">
								<c:if test="${office.level.id == OfficeLevel.BRANCHOFFICE.value && empty branch}">
									...
									<table width="95%" border="0" cellspacing="0" cellpadding="0">
										<tr>
											...
											<td width="39%" align="right">
												<html-el:link href="javascript:addNewOffice(5)">
								...
								<c:if test="${ !empty parentOffice  && parentOffice != office.parentOffice.officeName}">
									<br>
									<span class="fontnormal"><c:out value="${office.parentOffice.officeName}" /></span>
								</c:if>
								<c:if test="${empty parentOffice}">
									<span class="fontnormal"><c:out value="${office.parentOffice.officeName}" /></span>
									<c:set var="parentOffice" value="${office.parentOffice.officeName}"/>
