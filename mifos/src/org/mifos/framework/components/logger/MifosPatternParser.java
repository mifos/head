/** 
 * MifosPatternParser.java version:1.0
...
public class MifosPatternParser extends PatternParser {
	...
	/**
	 * Overriden method. If the specifier is 'L' it indicates the line number from where the log statement was issued
	 * If 'M' indicates the method name, 'C' indicates the class name. The MifosLocationPatternConverter will call the 
	...
	protected void finalizeConverter(char c){
		...
		case 'L':
		      pc = new MifosLocationPatternConverter(formattingInfo,
		...
		case 'C':
		      pc = new MifosLocationPatternConverter(formattingInfo,
		    		  CLASS_LOCATION_CONVERTER,extractPrecisionOption());
		      currentLiteral.setLength(0);
		      break;
		...
		/**
		 * This class extends from the pattern convertor. This calls the convert method. The convert method makes the
		 * decision as to whether it should return the line number or method name or the class name
		 */ 
		private class MifosLocationPatternConverter extends PatternConverter {
			/**Denotes whether the line number , method name or class name has to be returned*/
			int type;
			      ...
			      switch(type) {
				    ...
				    //calls the method to return class name
			          case CLASS_LOCATION_CONVERTER:
				    	...
				    	//depending on the precision, the rightmost token is retrieved. If %C{1} and the class name is
				    	  //org.mifos.framework.components.logger.MifosPattern, then the display would show MifosPattern as class name
		...
		}//end - class MifosPatternConverter
