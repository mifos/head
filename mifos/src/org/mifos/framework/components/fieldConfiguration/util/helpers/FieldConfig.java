		...
		if (fieldList != null && fieldList.size() > 0)
			for (FieldConfigurationEntity fieldConfigurationEntity : fieldList) {
				...
				if (parentfieldConfigurationEntity == null) {
					if (fieldConfigurationEntity.getMandatoryFlag().equals(
							FieldConfigurationConstant.YES)
							&& fieldConfigurationEntity.getHiddenFlag().equals(
									FieldConfigurationConstant.NO)
							&& fieldConfigurationEntity.getFieldName()
									.endsWith(fieldName)) {
						return true;
					}
				} else if ((parentfieldConfigurationEntity.getMandatoryFlag()
						.equals(FieldConfigurationConstant.YES)
						&& parentfieldConfigurationEntity.getHiddenFlag()
								.equals(FieldConfigurationConstant.NO)
						&& fieldConfigurationEntity.getFieldName().endsWith(
								fieldName) && fieldConfigurationEntity
						.getHiddenFlag().equals(FieldConfigurationConstant.NO))
						|| ((parentfieldConfigurationEntity.getMandatoryFlag()
								.equals(FieldConfigurationConstant.NO)
								&& parentfieldConfigurationEntity
										.getHiddenFlag().equals(
												FieldConfigurationConstant.NO)
								&& fieldConfigurationEntity.getFieldName()
										.endsWith(fieldName)
								&& fieldConfigurationEntity.getHiddenFlag()
										.equals(FieldConfigurationConstant.NO) && fieldConfigurationEntity
								.getMandatoryFlag().equals(
										FieldConfigurationConstant.YES)))) {
					return true;
				}
			}
