...
public class AuditConfigurtion {
	...
	public static boolean isObjectPropertiesToBeMerged(String entityType,
			...
			if (entity.getName().equalsIgnoreCase(name)) {
				if (parentName == null) {
					if (entity.getParentName() == null
							&& entity.getMergeProperties() != null
							&& entity.getMergeProperties().equalsIgnoreCase(
									"yes")) {
						return true;
					}
				} else {
					if (entity.getParentName().equalsIgnoreCase(parentName)
							&& entity.getMergeProperties() != null
							&& entity.getMergeProperties().equalsIgnoreCase(
									"yes")) {
						return true;
	...
	private Map<String,String> createColumnNames(EntityType entityType){
		...
		if (objectToBeLogged != null && objectToBeLogged.getEntities() !=null && objectToBeLogged.getEntities().length > 0) {
			Entity[] entities = objectToBeLogged.getEntities();
			for (Entity entity : entities) {
				if (entity !=null) {
					if(entity.getMergeProperties() != null
								&& entity.getMergeProperties().equalsIgnoreCase(
										"yes")){
						if (entity.getParentName() == null) {
	...
	private Map<String,String> callMethodToCreateValueMap(String methodName,Short localeId) throws  SystemException{
		...
		for (int i = 0; i < methods.length; i++) {
			if(methods[i].getName().equalsIgnoreCase(methodName)){
				try {
					valueMap=(Map<String,String>)methods[i].invoke(new AuditConfigurationPersistence(), new Object[]{localeId});
				} catch (Exception e) {
