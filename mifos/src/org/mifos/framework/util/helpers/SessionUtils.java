...
public class SessionUtils {
	...
	/**
	 * Sets the attribute in the session and the key is formed by appending 
	 * path to the key passed.
	 * These attributes would be removed from the session when 
	 * <code>doCleanUp(String path,HttpSession session)</code> is called
	 */
	public static void setRemovableAttribute(String key, Object value,
		...
		MifosLogManager.getLogger(LoggerConstants.FRAMEWORKLOGGER)
			.debug("An attribute being set in the session with key being " +
				path + "_" + key);
	}
	...
	public static Object getAttribute(String key, HttpSession session){
		MifosLogManager.getLogger(LoggerConstants.FRAMEWORKLOGGER)
			.debug("The key to be compared is "+ key);
		...
		if (sessionIsEmpty || null == key) {
			...
			while (keys.hasMoreElements()) {
				...
				MifosLogManager.getLogger(LoggerConstants.FRAMEWORKLOGGER)
					.debug("The attribute name with which it is trying " +
						"to compare is " + attributeKey);
				...
				if (attributeKey.equals(key)) {
					...
					MifosLogManager.getLogger(LoggerConstants.FRAMEWORKLOGGER)
						.debug("An exact match has been found with key being "+
							key + " and attribute key being" + attributeKey);
					return returnable;
				} else if (attributeKey.endsWith("_"+key)) {
					Object returnable = session.getAttribute(attributeKey);
					MifosLogManager.getLogger(LoggerConstants.FRAMEWORKLOGGER)
						.debug("An approximate match has been found with key " +
							"being " + key + " and attribute key being" +
							attributeKey);
					return returnable;
				} else {
	...
	/**
	 * Removes the attributes from the session where the key has 
	 * path as its prefix
	 */
	public static void doCleanUp(String path, HttpSession session) {
		MifosLogManager.getLogger(LoggerConstants.FRAMEWORKLOGGER)
			.debug("Clean up in session utils has been called");
		Enumeration keys = session.getAttributeNames();
		if (null != keys) {
			while (keys.hasMoreElements()) {
				String attributeKey = (String)keys.nextElement();
				if (attributeKey.startsWith(path)) {
					MifosLogManager.getLogger(LoggerConstants.FRAMEWORKLOGGER)
						.debug("The attribute being removed from session is" +
							attributeKey);
