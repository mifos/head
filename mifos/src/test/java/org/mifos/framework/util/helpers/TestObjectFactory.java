...
public class TestObjectFactory {
	...
	public static PersonnelBO getPersonnel(Session session, Short personnelId) {
		return (PersonnelBO) addObject(testObjectPersistence.getPersonnel(
				session, personnelId));
	}
	...
	public static CenterBO createCenterForTestGetLoanAccounts(
		...
		return createCenterForTestGetLoanAccounts(customerName, meeting,
				getFees());
	}
	...
	public static CenterBO createCenter(String customerName, MeetingBO meeting,
			List<FeeView> fees) {
		return createCenter(customerName, meeting, SAMPLE_BRANCH_OFFICE,
				PersonnelConstants.SYSTEM_USER, fees);
	}

	public static CenterBO createCenterForTestGetLoanAccounts(
			String customerName, MeetingBO meeting, List<FeeView> fees) {
		return createCenter(customerName, meeting, SAMPLE_BRANCH_OFFICE,
				PersonnelConstants.TEST_USER, fees);
	}
	...
	public static GroupBO createGroupUnderCenterForTestGetLoanAccountsInActiveBadStanding(
	...
	public static GroupBO createGroupUnderBranchWithMakeUser(
			String customerName, CustomerStatus customerStatus, Short officeId,
			MeetingBO meeting, Short loanOfficerId) {
		Short formedBy = PersonnelConstants.SYSTEM_USER;
		return createGroupUnderBranch(customerName, customerStatus, null,
				false, null, null, null, getFeesWithMakeUser(), formedBy,
	...
	public static ClientBO createClient(String customerName,
			CustomerStatus status, CustomerBO parentCustomer, Date startDate) {
		ClientBO client;
	...
	public static LoanOfferingBO createLoanOffering(String name,
		...
		return createLoanOffering(name, name.substring(0, 1), applicableTo,
				...
				defInstallments, interestType, intDedAtDisb, princDueLastInst,
				meeting);
	}
	...
	public static LoanOfferingBO createLoanOffering(Date currentTime,
			MeetingBO meeting) {
		return TestObjectFactory.createLoanOffering("Loan", "L", currentTime,
				meeting);
	}
	...
	public static LoanOfferingBO createLoanOffering(String name,
			String shortName, ApplicableTo applicableTo, Date startDate,
			PrdStatus offeringStatus, Double defLnAmnt, Double defIntRate,
			Short defInstallments, InterestType interestType,
			boolean interestDeductedAtDisbursement,
			boolean principalDueInLastInstallment, MeetingBO meeting,
			GraceType graceType) {
		...
		InterestTypesEntity interestTypes = new InterestTypesEntity(
				interestType);
		GLCodeEntity glCodePrincipal = (GLCodeEntity) StaticHibernateUtil
				.getSessionTL().get(GLCodeEntity.class, Short.valueOf("11"));

		GLCodeEntity glCodeInterest = (GLCodeEntity) StaticHibernateUtil
	...
	public static LoanOfferingBO createLoanOffering(String name,
			...
			boolean interestDeductedAtDisbursement,
			boolean principalDueInLastInstallment, MeetingBO meeting,
			GraceType graceType, String loanAmountCalcType,
			String noOfInstallCalcType) {
		PrdApplicableMasterEntity prdApplicableMaster = new PrdApplicableMasterEntity(
				applicableTo);
		ProductCategoryBO productCategory = TestObjectFactory
				.getLoanPrdCategory();
		...
		try {
			loanOffering = new LoanOfferingBO(getContext(), name, shortName,
					...
					principalDueInLastInstallment, new ArrayList<FundBO>(),
					new ArrayList<FeeBO>(), meeting, glCodePrincipal,
					glCodeInterest, loanAmountCalcType, noOfInstallCalcType);
		}
		catch (ProductDefinitionException e) {
			throw new RuntimeException(e);
		}
	...
	public static LoanOfferingBO createLoanOfferingFromLastLoan(String name,
			String shortName, ApplicableTo applicableTo, Date startDate,
			PrdStatus offeringStatus, Double defIntRate,
			InterestType interestType, boolean interestDeductedAtDisbursement,
			boolean principalDueInLastInstallment, MeetingBO meeting,
			GraceType graceType, LoanPrdActionForm loanPrdActionForm) {
		PrdApplicableMasterEntity prdApplicableMaster = new PrdApplicableMasterEntity(
				applicableTo);
		ProductCategoryBO productCategory = TestObjectFactory
				.getLoanPrdCategory();
		...
		try {
			loanOffering = new LoanOfferingBO(getContext(), name, shortName,
					productCategory, prdApplicableMaster, startDate, null,
					null, gracePeriodType, (short) 0, interestTypes,
					defIntRate, defIntRate, defIntRate, true,
					interestDeductedAtDisbursement,
					principalDueInLastInstallment, new ArrayList<FundBO>(),
					new ArrayList<FeeBO>(), meeting, glCodePrincipal,
					glCodeInterest, loanPrdActionForm);
		}
		catch (ProductDefinitionException e) {
			throw new RuntimeException(e);
		}
	...
	public static SavingsOfferingBO createSavingsProduct(String name,
			...
			Double recommendedAmount, RecommendedAmountUnit unit,
			Double intRate, Double maxAmtWithdrawl, Double minAmtForInt,
			SavingsType savingsType, InterestCalcType interestCalculation,
			MeetingBO intCalcMeeting, MeetingBO intPostMeeting) {
		return createSavingsProduct(name, name.substring(0, 1), applicableTo,
				startDate, status, recommendedAmount, unit, intRate,
				maxAmtWithdrawl, minAmtForInt, savingsType,
				interestCalculation, intCalcMeeting, intPostMeeting);
	}
	...
	public static SavingsOfferingBO createSavingsProduct(String name,
		...
		return createSavingsProduct(name, shortName, applicableTo, startDate,
				status, recommendedAmount, unit, intRate, maxAmtWithdrawl,
				minAmtForInt, savingsType, interestCalculation, intCalcMeeting,
				intPostMeeting, TestGeneralLedgerCode.BANK_ACCOUNT_ONE,
	...
	public static SavingsOfferingBO createSavingsProduct(String name,
			String shortName, ApplicableTo applicableTo, Date startDate,
			PrdStatus status, Double recommendedAmount,
			RecommendedAmountUnit recommendedAmountUnit, Double intRate,
			Double maxAmtWithdrawl, Double minAmtForInt,
		...
		RecommendedAmntUnitEntity amountUnit = new RecommendedAmntUnitEntity(
				recommendedAmountUnit);
		SavingsOfferingBO product;
		try {
			product = new SavingsOfferingBO(TestUtils.makeUserWithLocales(),
					name, shortName, productCategory, prdApplicableMaster,
					startDate, null, null, amountUnit, savingsTypeEntity,
					intCalType, intCalcMeeting, intPostMeeting, new Money(
							recommendedAmount.toString()), new Money(
							maxAmtWithdrawl.toString()), new Money(minAmtForInt
							.toString()), intRate, depglCodeEntity,
					intglCodeEntity);
		}
		catch (ProductDefinitionException e) {
			throw new RuntimeException(e);
	...
	public static SavingsOfferingBO createSavingsProduct(String offeringName,
		...
		return createSavingsProduct(offeringName, shortName, applicableTo,
				currentDate, PrdStatus.SAVINGS_ACTIVE, 300.0,
				RecommendedAmountUnit.PER_INDIVIDUAL, 24.0, 200.0, 200.0,
				savingsType, InterestCalcType.MINIMUM_BALANCE, meetingIntCalc,
				meetingIntPost);
	}
...
//GLCodeEntity glCode = ChartOfAccountsCache.get("31301").getAssociatedGlcode();
	...
	public static FeeBO createOneTimeAmountFee(String feeName,
		...
		FeeBO fee;
		try {
			fee = createOneTimeAmountFee(feeName, feeCategory, feeAmnt,
					feePayment, getUserContext());
		}
		catch (Exception e) {
			throw new RuntimeException(e);
		}
	...
	public static FeeBO createOneTimeAmountFee(String feeName,
			FeeCategory feeCategory, String feeAmnt, FeePayment feePayment,
			UserContext userContext) {
		GLCodeEntity glCode = (GLCodeEntity) StaticHibernateUtil.getSessionTL().get(
	...
	public static FeeBO createOneTimeRateFee(String feeName,
		...
		FeeBO fee;
		try {
			fee = new RateFeeBO(getUserContext(), feeName,
					new CategoryTypeEntity(feeCategory),
					new FeeFrequencyTypeEntity(FeeFrequencyType.ONETIME),
	...
	public static MeetingBO getNewMeetingForToday(RecurrenceType frequency,
		...
		Calendar calendar = new GregorianCalendar();
		return getNewMeeting(frequency, recurAfter, meetingType, WeekDay
				.getWeekDay((short) calendar.get(Calendar.DAY_OF_WEEK)));
	}
	...
	public static void cleanUpAccount(Integer accountId) {
		if (null != accountId) {
			...
			AccountBO account = (AccountBO) session.get(AccountBO.class,
					accountId);
	...
	public static ActivityContext getActivityContext() {
		if (activityContext == null) {
			...
			activityContext = new ActivityContext((short) 0, uc.getBranchId()
					.shortValue(), uc.getId().shortValue());
		}
	 ...
	 */
	private static UserContext getUserContext() throws SystemException,
	...
	public static AccountCheckListBO createAccountChecklist(Short prdTypeId,
		...
		List<String> details = new ArrayList<String>();
		details.add("item1");
		ProductTypeEntity productTypeEntity = (ProductTypeEntity) StaticHibernateUtil
...
//		loanOfering.updateLoanOfferingSameForAllLoan(loanOfering);
	...
	public static LoanAccountView getLoanAccountView(LoanBO loan) {
		return new LoanAccountView(loan.getAccountId(), loan.getLoanOffering()
				.getPrdOfferingName(), loan.getType(), loan.getLoanOffering()
				.getPrdOfferingId(), loan.getState(), loan
	...
	public static OfficeBO createOffice(OfficeLevel level,
			...
			throws Exception {
		OfficeBO officeBO = new OfficeBO(TestUtils.makeUserWithLocales(),
	...
	public static SavingsOfferingBO createSavingsProduct(String offeringName,
		...
		MeetingBO meetingIntCalc = createMeeting(getNewMeetingForToday(WEEKLY,
				EVERY_WEEK, CUSTOMER_MEETING));
		MeetingBO meetingIntPost = createMeeting(getNewMeetingForToday(WEEKLY,
				EVERY_WEEK, CUSTOMER_MEETING));
		return createSavingsProduct(offeringName, shortName,
				ApplicableTo.GROUPS, new Date(System.currentTimeMillis()),
				PrdStatus.SAVINGS_ACTIVE, 300.0, recommendedAmountUnit, 24.0,
				200.0, 200.0, SavingsType.VOLUNTARY,
				InterestCalcType.MINIMUM_BALANCE, meetingIntCalc,
	...
	public static SavingsOfferingBO createSavingsProduct(String productName,
			String shortName, Date currentDate) {
		MeetingBO meetingIntCalc = createMeeting(getTypicalMeeting());
		MeetingBO meetingIntPost = createMeeting(getTypicalMeeting());
		return createSavingsProduct(productName, shortName, currentDate,
				meetingIntCalc, meetingIntPost);
	}

	public static SavingsOfferingBO createSavingsProduct(String productName,
			String shortName, Date currentDate, MeetingBO meetingIntCalc,
			MeetingBO meetingIntPost) {
		return createSavingsProduct(productName, shortName,
				ApplicableTo.GROUPS, currentDate, PrdStatus.SAVINGS_ACTIVE,
