...
//		loanOffering.updateLoanOfferingSameForAllLoan(loanOffering);
		...
		addRequestParameter("loanAmount", loanOffering.getEligibleLoanAmountSameForAllLoan()
				.getDefaultLoanAmount().toString());
	...
	public void testSchedulePreviewFailureWhenLoanProductFrequencyChanges()
		...
		populateLoanAmountSameForAllLoan("1", eligibleLoanAmountRange.getMaxLoanAmount(),
				eligibleLoanAmountRange.getMinLoanAmount(),
				eligibleLoanAmountRange.getDefaultLoanAmount(), loanPrdActionForm);
		populateNoOfInstallSameForAllLoan("1", eligibleInstallmentRange
				.getMaxNoOfInstall().toString(), eligibleInstallmentRange
				.getMinNoOfInstall().toString(), eligibleInstallmentRange
	...
	public void testGetPrdOfferingsApplicableForCustomersWithMeeting()
		...
		assertEquals("Group", ((CustomerBO) SessionUtils.getAttribute(
				LoanConstants.LOANACCOUNTOWNER, request)).getDisplayName());
		assertEquals(4, ((List<LoanOfferingBO>) SessionUtils.getAttribute(
				LoanConstants.LOANPRDOFFERINGS, request)).size());
...
//	}
	...
	public void testLoanAccountFromLastLoan() throws Exception {
		request.setAttribute(Constants.CURRENTFLOWKEY, flowKey);
		LoanOfferingBO loanOffering = getLoanOfferingFromLastLoan("lastLoan",
				"lamt", ApplicableTo.GROUPS, WEEKLY, EVERY_WEEK);
		initPageParams(loanOffering);
		jumpToSchedulePreview();
		actionPerform();
		addRequestParameter(Constants.CURRENTFLOWKEY, (String) request
	...
	public void testLoanAccountFromLoanCycle() throws Exception {
		request.setAttribute(Constants.CURRENTFLOWKEY, flowKey);
		LoanOfferingBO loanOffering = getLoanOfferingFromLoanCycle("loanCycle",
				"lcyc", ApplicableTo.GROUPS, WEEKLY, EVERY_WEEK);
		initPageParams(loanOffering);
		jumpToSchedulePreview();
		actionPerform();
		addRequestParameter(Constants.CURRENTFLOWKEY, (String) request
	...
	private void modifyActionDateForFirstInstallment() throws Exception {
		...
		LoanScheduleEntityIntegrationTest.modifyData(installment, new Money("5.0"),
	...
	private LoanOfferingBO getLoanOfferingFromLastLoan(String name,
		...
		loanPrdActionForm = loanOfferingBO
				.populateNoOfInstallFromLastLoanAmount("2", new Integer("0"),
						new Integer("100"), new Integer("101"), new Integer(
								"200"), new Integer("201"), new Integer("300"),
						new Integer("301"), new Integer("400"), new Integer(
								"401"), new Integer("500"), new Integer("501"),
	...
	private void initSchedulePreviewPageParams(LoanOfferingBO loanOffering) {
		...
		schedulePreviewPageParams.put("noOfInstallments", loanOffering
				.eligibleNoOfInstall(group.getMaxLoanAmount(loanOffering),
						group.getMaxLoanCycleForProduct(loanOffering))
				.getDefaultNoOfInstall().toString());
		schedulePreviewPageParams.put("disbursementDate", DateUtils
				.getCurrentDate(((UserContext) request.getSession()
						.getAttribute("UserContext")).getPreferredLocale()));
		schedulePreviewPageParams.put("gracePeriodDuration", "1");
		schedulePreviewPageParams.put("businessActivityId", "1");		
	...
	public void testShouldGetGlimSpecificAttributes() throws Exception {
		...
		LoanAccountAction loanAccountAction = new LoanAccountAction(
				loanBusinessServiceMock, configurationBusinessServiceMock, new GlimLoanUpdater());
		...
		assertEquals(glimSessionAttributes, loanAccountAction
				.getGlimSpecificPropertiesToSet(new LoanAccountActionForm(),
						"1", customerMock, new ArrayList<ValueListElement>()));
	...
	public void testShouldPopulateClientDetailsFromLoan() throws Exception {
		...
		List<LoanAccountDetailsViewHelper> clientDetails = new LoanAccountAction()
				.populateClientDetailsFromLoan(Arrays.asList(clientMock1,
						clientMock2), Arrays.asList(loanMock), new ArrayList<ValueListElement>());
		assertEquals(Arrays.asList(clientDetails1, clientDetails2),
				clientDetails);
