...
public class TestSavingsHelper extends MifosIntegrationTest {
	...
	public void testGetNextInterestCalculationDateThreeMonths()
			throws Exception {
		Date date;
		Date resultDate;
	...
	public void testGetNextInterestPostingDateWithRecurOnEveryMonth()
			throws Exception {
		Date date;
		Date resultDate;
		Date accountActivationDate = getDate("05/07/2006");
		MeetingBO meeting = TestObjectFactory.getNewMeetingForToday(MONTHLY,
				EVERY_MONTH, SAVINGS_INTEREST_POSTING);
	...
	public void testGetNextInterestPostingDateWithRecurOnEveryFourMonths()
			throws Exception {
		Date date;
		Date resultDate;
		...
		MeetingBO meeting = TestObjectFactory.getNewMeetingForToday(MONTHLY,
				EVERY_FOUR_MONTHS, SAVINGS_INTEREST_POSTING);
	...
	public void testPrevInterestPostingDateWithRecurOnEveryMonth()
			throws Exception {
		Date date;
		Date resultDate;
		//	Date accountActivationDate = getDate("05/07/2006");
		...
		resultDate = helper.getPrevScheduleDate(getDate("01/07/2006"),
				getDate("31/01/2007"), meeting);
		date = getDate("31/12/2006");
		assertEquals(date, resultDate);

		resultDate = helper.getPrevScheduleDate(getDate("01/07/2006"),
				getDate("28/02/2007"), meeting);
		date = getDate("31/01/2007");
		assertEquals(date, resultDate);
	}

	public void testPrevInterestPostingDateWithRecurOnEveryFourMonths()
			throws Exception {
		//Date accountActivationDate = getDate("25/12/2007");
		MeetingBO meeting = TestObjectFactory.getNewMeetingForToday(MONTHLY,
				EVERY_FOUR_MONTHS, SAVINGS_INTEREST_POSTING);
		...
		for (Date currentDate : keys) {
			assertEquals(currentAndPrevDates.get(currentDate), helper.getPrevScheduleDate(activationDate,
					currentDate, meeting));
		}
	}
