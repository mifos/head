...
public class EnumPlugin implements PlugIn {
	...
	public void init(ActionServlet actionServlet, ModuleConfig config)
		...
		try {
			...
			for (Class cl : enumClassList) {
				servletContext.setAttribute(getName(cl), constantBuilder
						.buildMap(cl));
			}
		} catch (Exception e) {
	...
	private String getName(Class cl) {
		...
		String constantKeyName = className
	...
	List<Class> buildClasses(List<String> enumFileNameList)
		...
		try {
			...
			throw new EnumsNotLoadedException(
					ExceptionConstants.ENUMSNOTLOADEDEXCEPTION, cnfe, values);
		}
	...
	private static class EnumMapBuilder {
		...
		public Map buildMap(Class constantClass)
