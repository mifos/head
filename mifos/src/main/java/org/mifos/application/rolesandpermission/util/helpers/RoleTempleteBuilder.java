 ...
 * RoleTempleteBuilder.java    version: 1.0
...
public class RoleTempleteBuilder {
	...
	private void makeTable(List<ActivityEntity> l, Short aid, StringBuilder buff,
		...
		if (lst.size() > 0) {
			...
			for (int i = 0; i < lst.size(); i++) {
				...
				buff
						.append("<td width=\"3%\" bgcolor=\"#FFFFFF\" class=\"paddingleft05BottomBorder\"><input name=\""
								+ "activity("
								+ nameHelper++
								+ ")\""
	...
	public StringBuilder getRolesTemplete(List<ActivityEntity> l) {
		...
		for (int i = 0; i < li.size(); i++) {
			...
			buff.append("<td width=\"3%\"  ><input name=\"" + "activity("
					+ nameHelper++ + ")\""
	...
	private boolean makeCheckTable(List<ActivityEntity> l, Short id,
		...
		if (checked) {
			MifosLogManager.getLogger(LoggerConstants.ROLEANDPERMISSIONLOGGER)
					.debug("item with id= " + id + "is checked");
