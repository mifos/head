...
public class FinancialInitializer {
	 ...
	 * the custom CoA config file will be considered</em>.
	 * Using the custom config will be the only way to add new accounts after
		...
		for (GLAccount glAccount : coa.getGLAccounts()) {
			...
			if (null == accountId) {
				...
				if (account.getCategoryType() != glAccount.categoryType)
					throw new FinancialException(
							"category type change not supported");
	 ...
	 */
	private static boolean accountHierarchyMatch(COABO account1,
			GLAccount account2) {

		COAHierarchyEntity account1hierarchy = account1.getCoaHierarchy()
				.getParentAccount();
		if (null == account1hierarchy) {
			if (null == account2.parentGlCode)
				...
				logger
						.error("persisted account has no parent, but new account does");
				return false;
			}
		}
		...
		if (!account1parentGlCode.equals(account2.parentGlCode)) {
			logger.error("persistent account parent gl code was "
					+ account1parentGlCode
					+ ", but new account parent gl code was "
					+ account2.parentGlCode);
			return false;
		}
