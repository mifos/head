...
public class CustomFieldsBackfiller {
	...
	public void addCustomFieldsForExistingRecords(EntityType categoryType,
			Short levelId, CustomFieldDefinitionEntity customField)
		...
		// the id's retrieved
		if (categoryType.equals(EntityType.CLIENT)
				|| categoryType.equals(EntityType.GROUP)
			...
			List<CustomerBO> listOfCustomers = customerService
					.getCustomersByLevelId(levelId);
			if (null != listOfCustomers && listOfCustomers.size() != 0) {
				Iterator<CustomerBO> listOfCustomersIter = listOfCustomers
						.iterator();
				while (listOfCustomersIter.hasNext()) {
					CustomerBO customerBO = listOfCustomersIter.next();
					CustomerCustomFieldEntity customerCustomFieldEntity = new CustomerCustomFieldEntity(
							customField.getFieldId(), customField.getDefaultValue(), customerBO);
		...
		else if (categoryType.equals(EntityType.OFFICE)) // For Office
		{
			...
			List<OfficeBO> listOfOffices = officeService
			...
			if (null != listOfOffices && listOfOffices.size() != 0) {
				...
				while (listOfOfficesIter.hasNext()) {
					...
					OfficeCustomFieldEntity officeCustomFieldEntity = new OfficeCustomFieldEntity(
							customField.getDefaultValue(), customField.getFieldId(), officeBO);
		...
		else if (categoryType.equals(EntityType.PERSONNEL)) // For Personnel
		{
			...
			List<PersonnelBO> listOfPersonnel = personnelBusinessService
					.getAllPersonnel();
			if (null != listOfPersonnel && listOfPersonnel.size() != 0) {
				Iterator<PersonnelBO> listOfPersonnelIter = listOfPersonnel
						.iterator();
				while (listOfPersonnelIter.hasNext()) {
					PersonnelBO personnelBO = listOfPersonnelIter.next();
					PersonnelCustomFieldEntity personnelCustomFieldEntity = new PersonnelCustomFieldEntity(
							customField.getDefaultValue(), customField.getFieldId(), personnelBO);
		...
		else if (categoryType.equals(EntityType.SAVINGS)) // For Savings
		{
			...
			List<SavingsBO> listOfSavingsAccount = savingsBusinessService
					.getAllSavingsAccount();
			if (null != listOfSavingsAccount
					&& listOfSavingsAccount.size() != 0) {
				Iterator<SavingsBO> listOfSavingsAccountIter = listOfSavingsAccount
						.iterator();
				while (listOfSavingsAccountIter.hasNext()) {
					SavingsBO savingsBO = listOfSavingsAccountIter.next();
					AccountCustomFieldEntity accountCustomFieldEntity = new AccountCustomFieldEntity(
							savingsBO, customField.getFieldId(), customField.getDefaultValue());
		...
		else if (categoryType.equals(EntityType.LOAN)) // For Loan
		{
			...
			List<LoanBO> listOfLoanAccount = loanBusinessService
					.getAllLoanAccounts();
			if (null != listOfLoanAccount && listOfLoanAccount.size() != 0) {
				Iterator<LoanBO> listOfLoanAccountIter = listOfLoanAccount
						.iterator();
				while (listOfLoanAccountIter.hasNext()) {
					LoanBO loanBO = listOfLoanAccountIter.next();
					AccountCustomFieldEntity accountCustomFieldEntity = new AccountCustomFieldEntity(
							loanBO, customField.getFieldId(), customField.getDefaultValue());
					accountCustomFieldEntity.save(accountCustomFieldEntity);
				}
			}
		}
