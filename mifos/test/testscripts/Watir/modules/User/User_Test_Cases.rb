...
class User_Test_Cases < TestClass
  ...
  def Check_User_Links()
    begin    
         assert($ie.contains_text(@personnel_properties['Personnel.ViewUsers'])) and assert($ie.contains_text(@admin_properties['admin.definenewusers']))
 ...
 def   Man_New_User_with_password(password)    
  ...
  def create_User_duplicate_name(user_name, short_name, user_type, parent_user, address1 , address2, address3, city, state,
                                 country, postal_code, telephone, external_id, members_per_group, members_per_kendra, 
                                 distance_HO_to_BO)
         Click_Admin_page()
         Check_New_User()
