...
import org.mifos.framework.components.cronjobs.helpers.DuplicateClient;
...
public class TestDuplicateClientHelper extends MifosTestCase {
	...
	protected void setUp() throws Exception {
		super.setUp();
		duplicateClientHelper = new DuplicateClientHelper(new DuplicateClient());
		...
		client1 = TestObjectFactory.createClient("Client",
				ClientConstants.STATUS_ACTIVE, "1.1.1.1", group, startDate);
		client2 = TestObjectFactory.createClient("Client",
				ClientConstants.STATUS_ACTIVE, "1.1.1.1", group, startDate);
	...
	public void testexecute() throws Exception {
		...
		client1 = (ClientBO) testObjectPersistence.getObject(ClientBO.class,
				client1.getCustomerId());
		client2 = (ClientBO) testObjectPersistence.getObject(ClientBO.class,
				client2.getCustomerId());
		assertEquals(Short.valueOf(ClientConstants.STATUS_CLOSED), client1
				.getCustomerStatus().getId());
		assertEquals(Short.valueOf(ClientConstants.STATUS_CLOSED), client2
				.getCustomerStatus().getId());
		...
		testObjectPersistence.flushandCloseSession();
		duplicateClientHelper.execute(System.currentTimeMillis());
		client3 = (ClientBO) testObjectPersistence.getObject(ClientBO.class,
				client3.getCustomerId());
		client4 = (ClientBO) testObjectPersistence.getObject(ClientBO.class,
				client4.getCustomerId());

		assertEquals(Short.valueOf(ClientConstants.STATUS_CLOSED), client3
				.getCustomerStatus().getId());
		assertEquals(Short.valueOf(ClientConstants.STATUS_CLOSED), client4
				.getCustomerStatus().getId());
		group = (GroupBO) testObjectPersistence.getObject(GroupBO.class, group
				.getCustomerId());
		center = (CenterBO) testObjectPersistence.getObject(CenterBO.class,
