...
public class TestSavingsApplyAdjustmentAction extends MockStrutsTestCase{
	...
	public TestSavingsApplyAdjustmentAction() {
	}

	public TestSavingsApplyAdjustmentAction(String name) {
		super(name);
	}
	...
	public void testSuccessfullLoad_WithValidLastPaymentDeposit()throws Exception{
		...
		AccountPaymentEntity  payment = helper.createAccountPaymentToPersist(depositAmount,depositAmount,helper.getDate("20/05/2006"),AccountConstants.ACTION_SAVINGS_DEPOSIT,savings, createdBy,group);
		...
		assertNotNull(request.getSession().getAttribute(SavingsConstants.ACCOUNT_ACTION));
		AccountActionEntity accountAction = (AccountActionEntity)request.getSession().getAttribute(SavingsConstants.ACCOUNT_ACTION);
		assertEquals(AccountConstants.ACTION_SAVINGS_DEPOSIT,accountAction.getId().shortValue());
		assertEquals(Short.valueOf("1"),(Short)request.getSession().getAttribute(SavingsConstants.IS_LAST_PAYMENT_VALID));
	...
	public void testSuccessfullLoad_WithValidLastPaymentWithdrawal()throws Exception{
		...
		AccountPaymentEntity  payment = helper.createAccountPaymentToPersist(withdrawalAmount,balance,helper.getDate("20/05/2006"),AccountConstants.ACTION_SAVINGS_WITHDRAWAL,savings, createdBy,group);
		savings.addAccountPayment(payment);
		...
		assertNotNull(request.getSession().getAttribute(SavingsConstants.ACCOUNT_ACTION));
		AccountActionEntity accountAction = (AccountActionEntity)request.getSession().getAttribute(SavingsConstants.ACCOUNT_ACTION);
		assertEquals(AccountConstants.ACTION_SAVINGS_WITHDRAWAL,accountAction.getId().shortValue());
		assertEquals(Short.valueOf("1"),(Short)request.getSession().getAttribute(SavingsConstants.IS_LAST_PAYMENT_VALID));
		group=savings.getCustomer();
		center=group.getParentCustomer();
	}
	public void testSuccessfullLoad_WithoutValidLastPayment()throws Exception{
		...
		assertNull(request.getSession().getAttribute(SavingsConstants.ACCOUNT_ACTION));
		assertNull(request.getSession().getAttribute(SavingsConstants.CLIENT_NAME));
		assertEquals(Short.valueOf("0"),(Short)request.getSession().getAttribute(SavingsConstants.IS_LAST_PAYMENT_VALID));		
		group=savings.getCustomer();
		center=group.getParentCustomer();
	}
	...
	public void testSuccessfullPreviewSuccess()throws Exception{
		...
		HibernateUtil.closeSession();
		PersonnelBO createdBy = new PersonnelPersistence().getPersonnel(userContext.getId());
		AccountPaymentEntity  payment = helper.createAccountPaymentToPersist(new Money(Configuration.getInstance().getSystemConfig().getCurrency(),"1000.0"),new Money(Configuration.getInstance().getSystemConfig().getCurrency(),"1000.0"),helper.getDate("20/05/2006"),AccountConstants.ACTION_SAVINGS_DEPOSIT,savings, createdBy,group);
		savings.addAccountPayment(payment);
		savings.update();
		HibernateUtil.closeSession();
		request.getSession().setAttribute(Constants.BUSINESS_KEY,savings);
