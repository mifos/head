...
public class TestCheckList extends MifosMockStrutsTestCase {
	...
	protected void setUp() throws Exception {
		...
		try {
			setServletConfigFile(ResourceLoader.getURI(
					"org/mifos/META-INF/web.xml").getPath());
			setConfigFile(ResourceLoader.getURI(
					"org/mifos/META-INF/struts-config.xml").getPath());
	...
	private Short retriveValueObject(String checklistName) {
		...
		try {
			...
			Query query = session
					.createQuery("select max(checkList.checklistId) from org.mifos.application.checklist.util.valueobjects.CheckList checkList where checkList.checklistName = ?");
