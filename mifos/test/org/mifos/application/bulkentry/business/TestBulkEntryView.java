...
public class TestBulkEntryView extends TestCase {
	...
	public TestBulkEntryView() {
	}

	public TestBulkEntryView(String name) {
		super(name);
	}
	...
	public void testMultipleRepaymentAccountsForSingleProduct() throws SystemException, ApplicationException {
		...
		account1 = TestObjectFactory.createLoanAccount("42423142341",group,Short.valueOf("5"),new Date(System.currentTimeMillis()),loanOffering);
		account2 = TestObjectFactory.createLoanAccount("42423142341",group,Short.valueOf("5"),new Date(System.currentTimeMillis()),loanOffering);
		HibernateUtil.closeSession();
		...
		BulkEntryView groupBulkEntryView = parentBulkEntryView.getBulkEntryChildren().get(0);
		...
		assertEquals("The toal amount due for the product" , loanAccountsProductView.getTotalAmountDue().doubleValue(),424.0);
		HibernateUtil.closeSession();
		account1 =(LoanBO) new AccountPersistanceService().getAccount(account1.getAccountId());
		account2 = (LoanBO)new AccountPersistanceService().getAccount(account2.getAccountId());
	}
	...
	public void testMultipleRepaymentDisbursalAccountsForSingleProduct() throws SystemException, ApplicationException {
		...
		loanOffering = TestObjectFactory.createLoanOffering(
				...
				Short.valueOf("1"), meeting);
		account1 = TestObjectFactory.createLoanAccount("42423142341",group,Short.valueOf("5"),new Date(System.currentTimeMillis()),loanOffering);
		account2 = TestObjectFactory.createLoanAccountWithDisbursement("42423142341",group,Short.valueOf("3"),new Date(System.currentTimeMillis()),loanOffering,1);
		HibernateUtil.closeSession();
		...
		assertEquals("The size of loan Accounts" , loanAccountsProductView.getLoanAccountViews().size(),2);
		assertEquals("The amount due for loan account1",account1.getTotalAmountDue().getAmountDoubleValue(),212.0);
		assertEquals("The toal amount due for the product" , loanAccountsProductView.getTotalAmountDue().doubleValue(),242.0);
		assertEquals("The toal amount to be disbursed for the product" , loanAccountsProductView.getTotalDisburseAmount().doubleValue(),300.0);
		HibernateUtil.closeSession();
		account1 =(LoanBO) new AccountPersistanceService().getAccount(account1.getAccountId());
		account2 = (LoanBO)new AccountPersistanceService().getAccount(account2.getAccountId());
	}
	...
	public void testMultipleDisbursalAccountsForSingleProduct() throws SystemException, ApplicationException {
		...
		loanOffering = TestObjectFactory.createLoanOffering(
				...
				300.0, 1.2, Short.valueOf("3"), Short.valueOf("1"), Short
						.valueOf("1"), Short.valueOf("1"), Short.valueOf("1"),
				Short.valueOf("1"), meeting);
		account1 = TestObjectFactory.createLoanAccountWithDisbursement("42423142341",group,Short.valueOf("3"),new Date(System.currentTimeMillis()),loanOffering,1);
		account2 = TestObjectFactory.createLoanAccountWithDisbursement("42423142341",group,Short.valueOf("3"),new Date(System.currentTimeMillis()),loanOffering,1);
		HibernateUtil.closeSession();
		...
		assertEquals("The Size of products ",loanProducts.size(),1);
		assertEquals("The size of loan Accounts" , loanAccountsProductView.getLoanAccountViews().size(),2);
		assertEquals("The toal amount due for the product" , loanAccountsProductView.getTotalAmountDue().doubleValue(),60.0);
		assertEquals("The toal amount to be disbursed for the product" , loanAccountsProductView.getTotalDisburseAmount().doubleValue(),600.0);
		HibernateUtil.closeSession();
		account1 =(LoanBO) new AccountPersistanceService().getAccount(account1.getAccountId());
		account2 = (LoanBO)new AccountPersistanceService().getAccount(account2.getAccountId());
	}
	...
	private OfficeView getOfficeView(OfficeBO office) {
		OfficeView officeView = new OfficeView(office.getOfficeId(),office.getOfficeName(),office.getLevel().getLevelId(),office.getVersionNo());
		return officeView;
	}
	...
	public void testPopulateForCustomerAccount() {
		...
		CustomerAccountView customerAccountView = bulkEntryView.getCustomerAccountDetails();
		assertEquals("The retrieved accountId of the customer account should be equal to the created",center.getCustomerAccount().getAccountId(),
				customerAccountView.getAccountId());
