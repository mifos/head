...
DATABASE_VERSION=`echo 'select max(upgrade_id) from mifos_gazelle_acceptance.applied_upgrades' | mysql --skip-column-names -u hudson -phudson hudson_mifos_gazelle_trunk`
tempDir=`mktemp -d`
