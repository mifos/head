				...
				[
					...
					<div>
						...
						[#list client.groupLoanGlobalAccountNum as gLoan]
							<span>
								<a href='groupIndividualLoanAccountAction.do?method=get&globalAccountNum=${gLoan}&recordOfficeId=${client.branchId}&recordLoanOfficerId=${client.loanOfficerId?if_exists}&randomNUm=${Session.randomNUm?if_exists?c}'>
			...
			[#if customerHierarchy.loan?has_content]
				[
					...
					<div>
						...
						<span class='headingblue' style='font-size: 12px;'>
						<!-- client new glim -->
						[#if loan.accountStatusId == 5 && loan.clientName?has_content]
