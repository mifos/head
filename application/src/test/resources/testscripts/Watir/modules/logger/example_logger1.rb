#this is the class uses for log 
require 'watir' 
include Watir 
...
class LoggerFactory 
 ...
 def LoggerFactory.start_default_logger(fileNamePrefix)
  ...
  def LoggerFactory.start_xml_logger(fileNamePrefix) 
       ...
       logger = XMLLogger.new(File.join(File.dirname(__FILE__),'logs/', "#{fileNamePrefix}_#{time}.txt") ,2, 1000000, File.join(File.dirname(__FILE__),'logs/',"#{fileNamePrefix}_#{time}.XML"))
       return logger 
 end 
...
class XMLLogger < CoreLogger 
	...
	@logfile.puts "<?xml-stylesheet type=\"text/xsl\" href=\"test.xsl\"?>"
    ...
    def log_results(name, input, expected, test_status) 
         ...
         #@logfile.puts"</result>" 
	 @logfile.puts"<result>"
	 @logfile.puts"<testcase>"+name+"</testcase>"
	 @logfile.puts"<Input>"+input+"</Input>"
	 @logfile.puts"<Expected>"+expected+"</Expected>"
	 @logfile.puts"<Status>"+test_status+"</Status>"
	 @logfile.puts"</result>"
    end 
    def log_message(msg_txt)
	@logfile.puts '<result message="' +msg_txt +'"/>'
