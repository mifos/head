...
public class CollectionSheetReportServiceIntegrationTest extends AbstractCollectionSheetIntegrationTestCase {
    ...
    public void testGetCollectionSheetForGivenBranchLoanOfficerCenterAndMeetingDate() throws Exception {
        ...
        expect(
                collectionSheetServiceMock.getCollectionSheetForGroups(sqlMeetingDate, centerCollectionSheets.get(0),
                        LOAN_OFFICER_SHORT_ID)).andReturn(groupsCollectionSheets);

        expect(
                collectionSheetServiceMock.getCollectionSheetForCustomers(sqlMeetingDate,
                        groupsCollectionSheets.get(0), LOAN_OFFICER_SHORT_ID)).andReturn(customerCollectionSheets);
        ...
        List<CollectionSheetReportDTO> collectionSheets = collectionSheetReportService.getCollectionSheets(BRANCH_ID,
                LOAN_OFFICER_ID, CENTER_ID, sqlMeetingDate);
    ...
    private void setProductOfferingExpectation() throws ServiceException {
        setLoanOfferingExpectation(CollectionSheetReportServiceIntegrationTest.BSKL_PRODUCT_OFFERING_ID,
                CollectionSheetReportServiceIntegrationTest.CL_PRODUCT_OFFERING_ID);
        setSavingsOfferingExpectation(CollectionSheetReportServiceIntegrationTest.MM1_SAVINGS_PRODUCT_ID,
                CollectionSheetReportServiceIntegrationTest.MM2_SAVINGS_PRODUCT_ID);

    }

    private void setSavingsOfferingExpectation(Short savingProductOffering1, Short savingProductOffering2)
            throws ServiceException {
        expect(reportProductOfferingServiceMock.getSavingsOffering1()).andReturn(
                SavingsOfferingBO.createInstanceForTest(savingProductOffering1));
        expect(reportProductOfferingServiceMock.getSavingsOffering2()).andReturn(
                SavingsOfferingBO.createInstanceForTest(savingProductOffering2));
    }

    private void setLoanOfferingExpectation(Short loanProductOffering1, Short loanProductOffering2)
            throws ServiceException {
    ...
    public void testCollectionSheetForAllLoanOfficerAllCenterOffices() throws Exception {
        ...
        for (PersonnelBO loanOfficer : loanOfficers) {
            ...
            expect(
                    collectionSheetServiceMock.getCollectionSheetForGroups(sqlMeetingDate, centerCollectionSheet,
                            loanOfficer.getPersonnelId())).andReturn(groupsCollectionSheets);
            expect(
                    collectionSheetServiceMock.getCollectionSheetForCustomers(sqlMeetingDate, groupCollectionSheet,
                            loanOfficer.getPersonnelId())).andReturn(customerCollectionSheets);
            setProductOfferingExpectation();
        }
        expect(officeBusinessServiceMock.getOffice(BRANCH_SHORT_ID)).andReturn(OFFICE);
        replay(personnelBusinessServiceMock);
        ...
        replay(reportProductOfferingServiceMock);
        List<CollectionSheetReportDTO> collectionSheets = collectionSheetReportService.getCollectionSheets(BRANCH_ID,
                ALL_LOAN_OFFICER_ID, ALL_CENTER_ID, meetingDate);
        verify(personnelBusinessServiceMock);
        verify(customerBusinessServiceMock);
        verify(collectionSheetServiceMock);
        verify(officeBusinessServiceMock);
    ...
    public void testBsklProductFilterReturnsLoanProductWithBsklOfferingType() throws Exception {
        ...
        expect(reportProductOfferingServiceMock.getLoanOffering1()).andReturn(bsklLoanProductOffering);
        expect(accountBusinessServiceMock.getAccount(BSKL_LOAN_ACCNT_ID)).andReturn(
    ...
    public void testClLoanProductFilterReturnsLoanProductWithClLoanOfferingType() throws Exception {
        collectionSheet.addCollectionSheetLoanDetail(clLoanDetailsEntity);
        expect(reportProductOfferingServiceMock.getLoanOffering2()).andReturn(clLoanProductOffering);
        replay(reportProductOfferingServiceMock);
        expect(accountBusinessServiceMock.getAccount(CL_LOAN_ACCNT_ID)).andReturn(
                LoanBO.createInstanceForTest(clLoanProductOffering));
    ...
    public void testMm1SavingsProductFilterReturnsSavingsProductWithMm1OfferingType() throws Exception {
        collectionSheet.addCollectionSheetSavingsDetail(mm1SavingDetailsEntity);
        expect(reportProductOfferingServiceMock.getSavingsOffering1()).andReturn(mm1SavingsProductOffering);

        SavingsBO mm1SavingsBO = new SavingsBO();
        mm1SavingsBO.populateInstanceForTest(mm1SavingsProductOffering);
        expect(accountBusinessServiceMock.getAccount(MM1_SAVINGS_ACCNT_ID)).andReturn(mm1SavingsBO);
    ...
    public void testSkSavingsProductFilterReturnsSavingsProductWithMm1OfferingType() throws Exception {
        collectionSheet.addCollectionSheetSavingsDetail(skSavingDetailsEntity);
        expect(reportProductOfferingServiceMock.getSavingsOffering2()).andReturn(skSavingsProductOffering);

        SavingsBO skSavingsBO = new SavingsBO();
        skSavingsBO.populateInstanceForTest(skSavingsProductOffering);
        expect(accountBusinessServiceMock.getAccount(SK_SAVINGS_ACCNT_ID)).andReturn(skSavingsBO);
    ...
    protected void setUp() throws Exception {
        ...
        collectionSheetReportService = new CollectionSheetReportService(collectionSheetServiceMock,
                officeBusinessServiceMock, accountBusinessServiceMock, reportProductOfferingServiceMock,
        ...
        groupCollectionSheet = generateCollectionSheet(GROUP_ID, LOAN_OFFICER_SHORT_ID, CustomerLevel.GROUP);
        ...
        skSavingDetailsEntity = CollectionSheetSavingDetailsEntityFixture.createSavingsDetails(SK_SAVINGS_ACCNT_ID);
        ...
        mm1SavingsProductOffering = SavingsOfferingBO.createInstanceForTest(MM1_SAVINGS_PRODUCT_ID);
        skSavingsProductOffering = SavingsOfferingBO.createInstanceForTest(SK_SAVING_PRD_OFFERING_ID);

    }

    public void testGetReportDataReturnsListOfCollectionSheetReportData() throws Exception {
        expect(collectionSheetServiceMock.extractReportData(BRANCH_ID, meetingDate, PERSONNEL_ANY_ID, CENTER_ID))
                .andReturn(new ArrayList<CollectionSheetReportData>());
        replay(collectionSheetServiceMock);
        List<CollectionSheetReportData> reportData = collectionSheetReportService.getReportData(BRANCH_ID,
                meetingDateStr, PERSONNEL_ANY_ID, CENTER_ID);
        verify(collectionSheetServiceMock);
    }
