...
public class WebTierCenterDetailsServiceFacade implements CenterDetailsServiceFacade {
    ...
    @Override
    public CenterInformationDto getCenterInformationDto(String globalCustNum, Short levelId, UserContext userContext)
            throws ServiceException {

        CenterBO center = customerDao.findCenterBySystemId(globalCustNum);
        if (center == null) {
            throw new MifosRuntimeException("Center not found for globalCustNum, levelId: " + globalCustNum + ","
                    + levelId);
        }
