...
public class PovertyBand {
    ...
    public enum PovertyBandEnum {
        VERY_POOR("VeryPoor"), POOR("Poor"), AT_RISK("AtRisk"), NON_POOR("NonPoor");
    ...
    public static PovertyBand fromInt(int points, PPISurvey survey) throws Exception {
        for (PovertyBandEnum candidate : PovertyBandEnum.values()) {
            Method minMethod = survey.getClass().getMethod("get" + candidate.getValue() + "Min", new Class[0]);
            Method maxMethod = survey.getClass().getMethod("get" + candidate.getValue() + "Max", new Class[0]);
