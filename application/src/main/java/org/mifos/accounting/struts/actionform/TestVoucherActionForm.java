...
public class TestVoucherActionForm extends BaseActionForm {
	private static final Logger logger = LoggerFactory
			.getLogger(TestVoucherActionForm.class);
	...
	private ActionErrors mandatoryCheck(UserContext userContext) {
		 ...
		 * errors.add(SimpleAccountingConstants.MANDATORYENTER, new
		 * ActionMessage(SimpleAccountingConstants.MANDATORYENTER, Amount)); }
		 */
		/*
		...
		/*
		 * for(CoaNamesDto namesdto:coaNamesDtolist) { if ( namesdto.getAmount()
		 * == null || "".equals( namesdto.getAmount())) {
		 * errors.add(SimpleAccountingConstants.MANDATORYENTER, new
		 * ActionMessage(SimpleAccountingConstants.MANDATORYENTER, Amount));
		 *
		 * } if ( Integer.parseInt(namesdto.getAmount()) <0 ) {
		 *
		 * errors.add(SimpleAccountingConstants.MANDATORYENTER, new
		 * ActionMessage
		 ...
		 * Amount)); }
		 *
		 * } for(CoaNamesDto namesdto:coaNamesDtolist) { if (
		 * namesdto.getTransactionnotes() == null || "".equals(
		 * namesdto.getTransactionnotes())) {
		 * errors.add(SimpleAccountingConstants.MANDATORYFIELDS, new
		 * ActionMessage( SimpleAccountingConstants.MANDATORYFIELDS, Notes));
		 *
		 * }
		 *
		 * }
	 ...
	 * !DateUtils.isValidDate(getChequeDate())) errors =
	 * chequeDateValidate(errors, locale);
	 *
	 * return errors; }
	 */
	/*
	 * private ActionErrors chequeDateValidate(ActionErrors errors, Locale
	 * locale) { if (StringUtils.isNotBlank(getChequeDate()) &&
	 * !DateUtils.isValidDate(getChequeDate())) { ResourceBundle resources =
	 ...
	 * errors.add(SimpleAccountingConstants.INVALIDDATE, new
	 * ActionMessage(SimpleAccountingConstants.INVALIDDATE, chequeDate)); }
	 * return errors; }
	 */
