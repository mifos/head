...
public class AccountTagGenerator extends TagGenerator {
    ...
    protected StringBuilder build(AbstractBusinessObject obj, boolean selfLinkRequired, Object randomNum) {
        ...
        strBuilder.append(" / ");
        if(account.isGroupLoanAccountMember() == true){											// If individual loan account
        	createAccountLink(strBuilder, ((LoanBO)account).getParentAccount(), randomNum);		// create link to parent group loan account
        	strBuilder.append(" / ");
        }
        if (selfLinkRequired) {
