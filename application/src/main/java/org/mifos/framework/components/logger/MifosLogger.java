...
public abstract class MifosLogger {
     ...
     * @param asString
     *            Boolean value which decides if the parameter "key" should be
     *            treated as a key in the resource bundle or as a string
     * @param args
     *            List of parameters which can be used to replace placeholders
     *            in a message.
     */
     ...
     *            in a message.
     * @param t
     *            Exception object whose stack trace will be printed
     */
     ...
     *            in a message.
     * @param t
     *            Throwable object containing the exception
     */
    protected void logMessage(Level level, String key, boolean asString, Object[] args, Throwable t) {
        if (!asString) {
            // creating message object. the userId and the officeID will be
            // appended to the message string
            Message message = new Message(key, getUserID(), getOfficeID());

            logNonKey(level, message.toString(), t);
        } else {
            // if argument list is not empty then the key is used to retrieve
            // the string from the resource bundle
            // and the place holders are replaced. The userId and OfficeId are
            // also attached to the list of arguments.
     ...
     */
    protected abstract void logNonKey(Level level, String message, Throwable exception);

    /**
     * Low-level method to log, when a key is supplied (not a message).
     */
    protected abstract void logKey(Level level, String key, Object[] args1, Throwable exception);
