...
/**
 * This class has one method validate which validates the double value against
 * the format passed.
 */
public class DecimalFieldHelper {
    ...
    /**
     * This method validates that the number passed as value is less than or
     * equal to the number formed based on the format. E.g. if the format passed
     * is (10,3) the method would return if the number is less than or equal to
     * 9999999.999 else it would return false.
     * 
     * @param value
     *            -- value to be validated.
     * @param format
     *            -- format against which the value is to be validated e.g.
     *            (10,3)
     */
    public static boolean validate(Double value, String format) {
        ...
        // it gets the total possible digits by getting a substring from the
        // format
        ...
        MifosLogManager.getLogger(LoggerConstants.FRAMEWORKLOGGER).debug(
                "totalDigitsPossible is " + totalDigitsPossible);
        int possibleDigitsPostDecimal = Integer
                .parseInt(format.substring(format.indexOf(",") + 1, format.length() - 1));
        MifosLogManager.getLogger(LoggerConstants.FRAMEWORKLOGGER).debug(
                "possibleDigitsPostDecimal are " + possibleDigitsPostDecimal);
        int possibleDigitsBeforeDecimal = totalDigitsPossible - possibleDigitsPostDecimal;
        MifosLogManager.getLogger(LoggerConstants.FRAMEWORKLOGGER).debug(
                "possibleDigitsBeforeDecimal are " + possibleDigitsBeforeDecimal);
