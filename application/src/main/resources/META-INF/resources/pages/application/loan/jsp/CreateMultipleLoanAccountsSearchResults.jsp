	...
	<tiles:put name="body" type="string">
		...
		<script>
			...
			function fun_saveForLater(form) {
				...
				form.submit();
				func_disableSubmitBtn("saveFomultipleloansactionrLaterButton");
			}
										...
										<tr>
											<td width="50%">
												<table border="0" cellspacing="0" cellpadding="0">
													<tr>
														<td><img
															src="pages/framework/images/timeline/tick.gif" width="17"
															height="17"></td>
									...
									<table width="98%" border="0" cellspacing="0" cellpadding="3">
										<tr>
											<td class="headingorange"><span class="heading"><mifos:mifoslabel
														name="loan.create" /> <mifos:mifoslabel
										...
										<tr>
											<td class="fontnormalbold"><span class="fontnormal"><mifos:mifoslabel
													...
													name="${ConfigurationConstants.LOAN}" /> <mifos:mifoslabel
													name="loan.instancename" isColonRequired="Yes" /> <c:set
													var="loanPrdOffering"
													value="${session:getFromSession(sessionScope.flowManager,requestScope.currentFlowKey,'loanOffering')}"
													scope="page" /> <span class="fontnormal"> <c:out
														value="${loanPrdOffering.prdOfferingName}" />
											</span></td>
										</tr>
									</table> <font class="fontnormalRedBold"> <span
										id="CreateMultipleLoanAccountsSearchResults.error.message">
											<html-el:errors bundle="loanUIResources" />
									</span></font> <br> <!-- begin search results list -->
									<table id="SearchClients" width="96%" border="0"
										cellpadding="3" cellspacing="0">
										<tr>
											<td width="5%" valign="top" class="drawtablerowboldnolinebg">
												<input
												id="CreateMultipleLoanAccountsSearchResults.input.selectAll"
												type="checkbox" onclick="selectAll(this)" name="selectAll1">
											</td>
											<td width="29%" class="drawtablerowboldnolinebg"><mifos:mifoslabel
													name="loan.acc_owner" /></td>
											<td width="15%" class="drawtablerowboldnolinebg"><mifos:mifoslabel
													name="${ConfigurationConstants.LOAN}" /> <mifos:mifoslabel
													name="loan.amt" /></td>
											<td width="15%" class="drawtablerowboldnolinebg"><mifos:mifoslabel
													name="loan.allowed_amount1" /></td>
											<td width="15%" class="drawtablerowboldnolinebg"><mifos:mifoslabel
													name="loan.no_of_inst" /></td>
										...
										<c:forEach var="clientDetail" varStatus="loopStatus"
											items="${sessionScope.multipleloansactionform.clientDetails}">
											<tr>
												<td valign="top" class="drawtablerow"><html-el:checkbox
														property="clientDetails[${loopStatus.index}].selected"
														value="true" onclick="selectAllCheck(this)"
														styleId="CreateMultipleLoanAccountsSearchResults.checkbox.${loopStatus.index}" />
												</td>
												<td width="29%" valign="top" class="drawtablerow"><span
													class="fontnormalbold"><mifos:mifoslabel
															name="${ConfigurationConstants.CLIENT}"
															isColonRequired="Yes" /></span> <span
													id="CreateMultipleLoanAccountsSearchResults.clientName.${loopStatus.index}"><c:out
															value="${clientDetail.client.displayName}" /></span>: <mifos:mifoslabel
														name="${ConfigurationConstants.ID}" /> <c:out
														value="${clientDetail.client.globalCustNum}" /> <br>
													<span class="fontnormalbold"><mifos:mifoslabel
															name="bulkEntry.loanofficer" isColonRequired="Yes" /></span> <c:out
														value="${clientDetail.client.personnel.displayName}" /> <br>
													<c:out value="${clientDetail.client.office.officeName}" />
													/ <c:out
														value="${clientDetail.client.parentCustomer.parentCustomer.displayName}" />
													/ <c:out
														value="${clientDetail.client.parentCustomer.displayName}" />
												</td>
												<td width="15%" valign="top" class="drawtablerow"><html-el:text
														styleId="CreateMultipleLoanAccountsSearchResults.input.loanAmount"
														property="clientDetails[${loopStatus.index}].loanAmount"
														styleClass="separatedNumber" /></td>
												<td width="15%" valign="top" class="drawtablerow"><span
													id="CreateMultipleLoanAccountsSearchResults.minLoanAmount.${loopStatus.index}"><fmt:formatNumber
															value="${clientDetail.minLoanAmount.amount}" /></span> &nbsp; -
													&nbsp; <span
													id="CreateMultipleLoanAccountsSearchResults.maxLoanAmount.${loopStatus.index}"><fmt:formatNumber
															value="${clientDetail.maxLoanAmount.amount}" /></span></td>
												<td width="15%" valign="top" class="drawtablerow"><span
													id="CreateMultipleLoanAccountsSearchResults.NoOfInstall.${loopStatus.index}"><fmt:formatNumber
									...
									<table width="96%" border="0" cellpadding="0" cellspacing="0">
										<tr>
											<td align="center"><html-el:button
													styleId="CreateMultipleLoanAccountsSearchResults.button.saveForLater"
													property="saveForLaterButton" styleClass="buttn"
													onclick="javascript:fun_saveForLater(this.form)">
													<mifos:mifoslabel name="loan.saveForLater" />
												</html-el:button> &nbsp; <c:set
													value="${session:getFromSession(sessionScope.flowManager,requestScope.currentFlowKey,'pendingApprovalDefined')}"
													var="PendingApproval" /> <c:choose>
													<c:when test='${PendingApproval == true}'>
														<html-el:button property="submitForApprovalButton"
															styleClass="buttn submit"
															onclick="javascript:fun_submitForApproval(this.form)"
															styleId="CreateMultipleLoanAccountsSearchResults.button.submit">
															<mifos:mifoslabel name="loan.submitForApproval" />
														</html-el:button>
													</c:when>
													<c:otherwise>
														<html-el:button
															styleId="CreateMultipleLoanAccountsSearchResults.button.approve"
															property="approvedButton" styleClass="buttn submit"
															onclick="javascript:fun_approved(this.form)">
															<mifos:mifoslabel name="loan.approved" />
														</html-el:button>
													</c:otherwise>
												</c:choose> &nbsp; <html-el:button
													styleId="CreateMultipleLoanAccountsSearchResults.button.cancel"
													property="cancelButton" styleClass="cancelbuttn"
													onclick="location.href='multipleloansaction.do?method=cancel&currentFlowKey=${requestScope.currentFlowKey}'">
