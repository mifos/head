...
public class PersonnelDaoHibernate implements PersonnelDao {
    ...
    public MifosUser findAuthenticatedUserByUsername(String username) {
        ...
        return new MifosUser(user.getPersonnelId(), user.getOffice().getOfficeId(), user.getLevelEnum().getValue(), new ArrayList<Short>(roleIds), username, password,user.getHash(), enabled, accountNonExpired, credentialsNonExpired, accountNonLocked, authorities);
    }
