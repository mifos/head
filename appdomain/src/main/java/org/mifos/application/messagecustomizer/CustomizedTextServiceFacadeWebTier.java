...
public class CustomizedTextServiceFacadeWebTier implements CustomizedTextServiceFacade {
	...
	/*
	 * Take an input message and replace any instances of customized text strings that
	 * are found in the message.  If there end up being any performance issues around
	 * this method (for example, if the logic for this method needs to be more complex 
	 * --regular expressions, etc.) then this method is a good candidate for using the
	 * Spring @Cacheable annotation mechanism
	 * 
	...
	/*
	 * This method is used as the second step in migrating legacy customized labels to
	 * the new customized text mechanism.  Changeset MIFOS-4633_2 copies existing custom
	 * labels to the new customized_text table with the original_text field as the
	 * entity name + ".Label".  This was needed since the Liquibase conversion doesn't
	 * know what the configured locale is for Mifos or how to lookup properties values.
	 * 
	 * An example of possible resulting migrated data would be:
	 * original_text: Client.Label
	 * custom_text: Borrower
	 * 	 
	 * So we know that the "Client" entity label was customized.  The method below will
	 * check for original_text entries that have been migrated (ie. end in ".Label" and
	 * lookup the localized text that was the default label for that entity.  It replaces
	 * the migrated original_text with the localized text.
	 * 
	 * Finally it checks to see if there are cases where original_text maps to custom_text
	 * where both are the same (this would happen if in the original
	 */
	@Override
